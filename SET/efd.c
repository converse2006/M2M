...
static int parse_plt_for_arm(EFD *efd)
{
    ...
    fread(reltab, efd->sec_hdr[ndx_relplt].sh_size, 1, efd->pfile);
    ...
    /* Sanity check */
    if (efd->ndx_dynsym != efd->sec_hdr[ndx_relplt].sh_link)
        return -1;
...
EFD *efd_open_elf(char *name)
{
    ...
    /* Read ELF header */
    fread(&efd->elf_hdr, sizeof(Elf32_Ehdr), 1, efd->pfile);
    if (feof(efd->pfile))
    {
    ...
    fread(efd->sec_hdr, sizeof(Elf32_Shdr), efd->sec_num, efd->pfile);
    ...
    fread(efd->shstrtab, efd->sec_hdr[efd->ndx_shstr].sh_size, 1, efd->pfile);
    ...
    efd->sym_num = efd->sec_hdr[efd->ndx_sym].sh_size / efd->sec_hdr[efd->ndx_sym].sh_entsize;
    efd->symtab = (Elf32_Sym *) malloc(efd->sec_hdr[efd->ndx_sym].sh_size);

    fseek(efd->pfile, efd->sec_hdr[efd->ndx_sym].sh_offset, SEEK_SET);
    fread(efd->symtab, efd->sec_hdr[efd->ndx_sym].sh_size, 1, efd->pfile);
    ...
    fread(efd->strtab, efd->sec_hdr[efd->ndx_str].sh_size, 1, efd->pfile);
    ...
    else
    {
        ...
        efd->dynsym_num = efd->sec_hdr[efd->ndx_dynsym].sh_size / efd->sec_hdr[efd->ndx_dynsym].sh_entsize;
        ...
        fread(efd->dyntab, efd->sec_hdr[efd->ndx_dyn].sh_size, 1, efd->pfile);

        while (efd->dyntab[efd->lib_num].d_tag == DT_NEEDED)
            efd->lib_num++;
    }
...
void efd_close(EFD *efd)
{
    ...
    if (efd->dynsymtab != NULL)
        free(efd->dynsymtab);

    if (efd->dynstrtab != NULL)
        free(efd->dynstrtab);
...
uint16_t efd_find_sec_by_name(EFD *efd, char *name)
{
    ...
    for (i = 1; i < efd->sec_num; i++)
    {
        if (strcmp(name, efd_get_sec_name(efd, i)) == 0)
            return i;
    }
...
uint32_t efd_find_sym_by_name(EFD *efd, char *name)
{
    ...
    {
        if (strcmp(name, efd_get_sym_name(efd, i)) == 0)
            return efd_get_sym_value(efd, i);
    }
