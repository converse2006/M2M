...
#ifdef CONFIG_VPMU
    //sysbus_create_simple("vpmu", VPMU_BASE_ADDR, NULL);
    //vpmu_init(VPMU_BASE_ADDR, 31);
    // Added IRQ number (31) for VPMU by Chiaheng. 2010' 8' 19.
    {
        struct goldfish_device *vpmu_device;
        vpmu_device = qemu_mallocz(sizeof(*vpmu_device));
        vpmu_device->name = "vpmu";
        vpmu_device->id = 0;
        vpmu_device->base = VPMU_BASE_ADDR;
        vpmu_device->size = VPMU_IOMEM_SIZE;
        vpmu_device->irq = 31;
        vpmu_device->irq_count = 1;
        goldfish_add_device_no_io(vpmu_device);
        vpmu_init(vpmu_device->base, goldfish_pic[vpmu_device->irq]);
    }
#endif
...
#include "SET_device.h"
    /* drhydra added */
    /* Register SET */
    //sysbus_create_simple("set", SET_BASE_ADDR, NULL);
    SET_device_init(SET_BASE_ADDR, NULL);
#endif
