...
M2M_ERR_T m2m_dbp_init()
{
    ...
    {
        DB_data_base = GlobalVND.Shared_memory_address;
        DB_meta_base = GlobalVND.Shared_memory_address + (DATA_BUFFER_SIZE);
        ...
        HQ_data_base = GlobalVND.Shared_memory_address + TOTAL_DATA_BUFFER_SIZE;
        HQ_meta_base = GlobalVND.Shared_memory_address + TOTAL_DATA_BUFFER_SIZE + (HEADER_QUEUE_SIZE); 
...
//#endif
    ...
    else if(!strcmp(GlobalVND.DeviceType, "ZR") || !strcmp(GlobalVND.DeviceType, "ZC"))
    {
        ...
        DB_meta_base = GlobalVND.Shared_memory_address + (TOTAL_HEADER_QUEUE_SIZE * NODE_MAX_LINKS) + DATA_BUFFER_SIZE;
            ...
            m2m_hq_meta_start[HEADER_QUEUE_ENTRY_NUM]   = GlobalVND.Shared_memory_address + \
                          (TOTAL_HEADER_QUEUE_SIZE * NODE_MAX_LINKS) + TOTAL_DATA_BUFFER_SIZE + HEADER_QUEUE_SIZE; 
        }
//#ifdef M2MDEBUG
        for(index_link = 0; index_link < NODE_MAX_LINKS; index_link++)
...
GlobalVND.DeviceID, index_link, index_ent, m2m_hq_buffer_start[index_link][index_ent] - GlobalVND.Shared_memory_address);
        ...
        for(index_ent = 0; index_ent < HEADER_QUEUE_ENTRY_NUM; index_ent++)
            ...
            HEADER_QUEUE_ENTRY_NUM, index_ent, m2m_hq_buffer_start[NODE_MAX_LINKS][index_ent] - GlobalVND.Shared_memory_address);
            fprintf(stderr, "[%s][%d] m2m_hq_meta_start[%d] = %ld \n", GlobalVND.DeviceType, GlobalVND.DeviceID, \
                  HEADER_QUEUE_ENTRY_NUM, m2m_hq_meta_start[HEADER_QUEUE_ENTRY_NUM] - GlobalVND.Shared_memory_address);
        /*for(index = 0; index < DATA_BUFFER_ENTRY_NUM; index++)
            fprintf(stderr, "[%3s][%2d] m2m_dbp_buffer_start[%2d] = %d \n", GlobalVND.DeviceType, GlobalVND.DeviceID, \
                                                index, m2m_dbp_buffer_start[index] - GlobalVND.Shared_memory_address);
        for(index = 0; index < DATA_BUFFER_ENTRY_NUM; index++)
            fprintf(stderr, "[%3s][%2d] m2m_dbp_meta_start[%2d] = %d \n", GlobalVND.DeviceType, GlobalVND.DeviceID, \
                                                   index, m2m_dbp_meta_start[index] - GlobalVND.Shared_memory_address);
            fprintf(stderr, "[%3s][%2d] m2m_hq_buffer_start[%2d] = %d \n", GlobalVND.DeviceType, GlobalVND.DeviceID, \
                HEADER_QUEUE_ENTRY_NUM, m2m_hq_buffer_start[HEADER_QUEUE_ENTRY_NUM] - GlobalVND.Shared_memory_address);
            fprintf(stderr, "[%3s][%2d] m2m_hq_meta_start[%2d] = %d \n", GlobalVND.DeviceType, GlobalVND.DeviceID, \
                  HEADER_QUEUE_ENTRY_NUM, m2m_hq_meta_start[HEADER_QUEUE_ENTRY_NUM] - GlobalVND.Shared_memory_address);*/
        }
//#endif
