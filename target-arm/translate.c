...
#ifdef CONFIG_SET
        /* drhydra added */
        gen_helper_branch_detector(var, tcg_const_i32(s->pc));
#endif
    }
...
#ifdef CONFIG_SET
    /* drhydra added */
    gen_helper_branch_detector(tmp, tcg_const_i32(s->pc));
#endif
...
#ifdef CONFIG_VPMU
    gen_helper_dinero_access(addr , tcg_const_i32(D4XREAD), tcg_const_i32(1) );
#endif
    return tmp;
}
...
#ifdef CONFIG_VPMU
    gen_helper_dinero_access(addr , tcg_const_i32(D4XREAD), tcg_const_i32(2) );
#endif
    return tmp;
}
...
#ifdef CONFIG_VPMU
    gen_helper_dinero_access(addr , tcg_const_i32(D4XREAD), tcg_const_i32(4) );
#endif
    return tmp;
}
...
#ifdef CONFIG_VPMU
    gen_helper_dinero_access(addr , tcg_const_i32(D4XWRITE), tcg_const_i32(1) );
#endif
    dead_tmp(val);
}
...
#ifdef CONFIG_VPMU
    gen_helper_dinero_access(addr , tcg_const_i32(D4XWRITE), tcg_const_i32(2) );
#endif
    dead_tmp(val);
}
...
#ifdef CONFIG_VPMU
    gen_helper_dinero_access(addr , tcg_const_i32(D4XWRITE), tcg_const_i32(4) );
#endif
    dead_tmp(val);
}
...
static int disas_cp_insn(CPUState *env, DisasContext *s, uint32_t insn)
{
    ...
    if(!init)
    {
        struct cp14_pmu_state *cp14_state;
        cp14_state = (struct cp14_pmu_state *) qemu_mallocz(sizeof(struct cp14_pmu_state));
        // Added by Chiaheng for VPMU for Oprofile.
#ifdef CONFIG_VPMU_OPROFILE
        cp14_pmu_state_pt = cp14_state;
#endif
        cp14_state->vpmu_t = &GlobalVPMU;
        cpu_arm_set_cp_io(env, 14, pas_cp14_read, pas_cp14_write, cp14_state);
        init++;
    }
#endif
...
#ifdef CONFIG_SET
        /* drhydra added */
        gen_helper_branch_detector(tcg_const_i32(dest), tcg_const_i32(s->pc));
#endif
...
static int disas_coproc_insn(CPUState * env, DisasContext *s, uint32_t insn)
{
    ...
    case 11:
#ifdef CONFIG_VPMU
	s->tb->extra_tb_info.arm_count[ARM_INSTRUCTIONS + 1] += 1;//tianman
#endif
...
#ifdef CONFIG_VPMU
        /* paslab : disas_cp14_ read/write was default data flow ,but we goto disas_cp_insn for support */
        goto board;
#endif
        if (insn & (1 << 20))
...
static void disas_arm_insn(CPUState * env, DisasContext *s)
{
    ...
    if(vpmu_simulator_status(&GlobalVPMU, VPMU_PIPELINE_SIM)) 
	{
		...
		{
			//tianman
			//analyze_arm_ticks(insn, env, s);

			//evo0209
			s->tb->extra_tb_info.ticks += get_arm_ticks(insn, env, s);
			//printf("%u\n", get_arm_ticks(insn, env, s));
			//printf("%u\n", s->tb->extra_tb_info.ticks);
			if (s->tb->extra_tb_info.insn_buf_index == 2)
				dual_issue_check(s);
		}
		else
		{
			/* paslab : data sheet model */
			s->tb->extra_tb_info.ticks += get_insn_ticks( insn );
		}
	}
    if(vpmu_simulator_status(&GlobalVPMU, VPMU_INSN_COUNT_SIM)) 
	{
        /* shocklink added to support all changes to PC for portability */
        get_insn_parser(insn, &s->tb->extra_tb_info);
    }
#endif
...
#ifdef CONFIG_VPMU
            gen_helper_dinero_access( tcg_const_i32(insn), tcg_const_i32(0xff),tcg_const_i32(32));
#endif
            return; /* PLD */
	}
...
#ifdef CONFIG_VPMU
        if( ((insn>>24) & 0xf) == 0xa || ((insn>>24) & 0xf) == 0xb )
            gen_helper_branch_predictor(tcg_const_i32(99));
#endif
...
#ifdef CONFIG_VPMU
                /* paslab : support branch event (BX count) */
...
#ifdef CONFIG_VPMU
            /* paslab : support branch event (BLX count) */
            s->tb->extra_tb_info.branch_count += 1;
#endif
            tmp = load_reg(s, rm);
            tmp2 = new_tmp();
        ...
        case 0xb:
            {
                ...
                s->tb->extra_tb_info.branch_count += 1;
                if(cond != 0xe)
                    gen_helper_branch_predictor(tcg_const_i32(0));
#endif
                int32_t offset;
...
#ifdef CONFIG_SET
            /* drhydra added */
            gen_helper_swi_detector(tcg_const_i32(s->pc));
#endif
            break;
        default:
        illegal_op:
...
static int disas_thumb2_insn(CPUState *env, DisasContext *s, uint16_t insn_hw1)
{
    ...
    if(vpmu_simulator_status(&GlobalVPMU, VPMU_PIPELINE_SIM)) {
        /* paslab : data sheet model */
        s->tb->extra_tb_info.ticks += get_insn_ticks_thumb(insn);
    }
...
#ifdef CONFIG_VPMU
    /* paslab: initiate extra_tb_info struct */
    tb_extra_info_init( &(tb->extra_tb_info) , tb->pc );
    /* paslab : need helper implementation */
//  fprintf(stderr,"tb_extra_info_ptr = 0x%x\n", &(tb->extra_tb_info));
    gen_helper_vpmu_calculate(tcg_const_i32((uint32_t)&(tb->extra_tb_info)));
#endif
...
#ifdef CONFIG_VPMU
            /* paslab : instruction count support */
            tb->extra_tb_info.insn_count += 1;

            /* the instruction number inside one TB is at most 85 by simple test
             * is unlikely over 255
             * gkk886: it is over 255 when android boot, change char to int in vpmu.h
             */
            //if(unlikely(tb->extra_tb_info.insn_count == 255))
            //    fprintf(stderr,"TB insn count is over 255\n");
#endif
        }
        if (num_temps) {
...
done_generating:
    ...
    *gen_opc_ptr = INDEX_op_end;
    ...
    tb->extra_tb_info.icache_ref.address = pc_start;
    tb->extra_tb_info.icache_ref.size = dc->pc - pc_start;
    tb->extra_tb_info.icache_ref.accesstype = D4XINSTRN;
#endif
